...
Status: DONE (2026-09-26, accepted by the orchestrator; see the wave 3 review in README.md) · Depends on: 006, 017, 007a, 009a, 020 (all in `gcc-demo`) · Can run in parallel with: 015 and 013
...
- [x] 1.1 **`Callout`**: `variant: 'route' | 'block' | 'verdict' | 'stale'`, `title`, `children`, optional `action`. Copy rule for `block`: the body ends "This is a hard block, not a warning." (ui-direction §6.2). Tones from `tokens.css`; icon plus words, never colour alone.
- [x] 1.2 **`SourceChip` and `SourceHost`** (spec §5.3, ui-direction §7.4):
  - [x] 1.2.1 `SourceChip({ source, doc? })` renders the mono label (`p. 14`, `add.2 p. 3`, `Q-0412`, `Cred: GOSI`, `Calc: fit model`), quiet (`--ink-3`) until hover or focus. It accepts 007a's `SourceRef` (`kind: 'page' | 'credential' | 'project' | 'calc'`) plus `'addendum' | 'quote' | 'input'`, as a widened local type. Don't edit 007a's type.
  - [x] 1.2.2 Click on a `page` or `addendum` chip with a `doc` (`{ url, title }`): opens `PdfViewer` at that page with `terms` highlighted, through `SourceHost`, a provider mounted once in `Workspace` (and on `/dev/kit`) that lazy-loads the viewer. Without a `doc` (most tenders have no PDF in the demo), the chip is a button that shows a `Tip`: "Page 14 of the tender documents. This demo holds a copy of the hero tender only."
  - [x] 1.2.3 Click on `credential`, `project`, `quote`, `input` or `calc`: a `Tip` with the record's label and the one line the caller passes (`detail`). The Company vault page arrives with plan 010; don't link to it.
  - [x] 1.2.4 Keyboard: Enter or Space opens; Esc closes the viewer and returns focus to the chip.
- [x] 1.3 **`RecommendationCard`** (spec §5.1): props `agent`, `verdict` (text) plus a tone, `confidence` (band or level text), `confidenceWhy?`, `reasons` (at most 3 shown), `wouldChange[]`, `sources: SourceRef[]` (rendered as `SourceChip`s), `doc?`, `overriddenBy?: { name; at; reason }`, `children?` (the caller's actions). It always ends with the words "Recommendation, not a decision." When `overriddenBy` is set, it shows "Overridden by {name} at {time}: {reason}" under the verdict, with the original verdict still visible (never replaced).
...
- [x] 1.4 **`ReasonCodePicker`**: `codes: { id; label }[]`, `value: { codes: string[]; note: string }`, `required: 'none' | 'code' | 'note' | 'code-or-note'`, `onChange`. It reports its own missing-reason sentence (for the caller's disabled button): "Pick a reason or write a note."
- [x] 1.5 **`OverrideModal`** on `ModalFrame`: props `title`, `from` (the recommendation), `options` (the choices, the opposite pre-selected), `reasonRule` (passed to `ReasonCodePicker`), `codes?`, `consequence?` (e.g. "Overriding does not by itself pursue the tender."), `onConfirm({ choice, codes, note })`. Copy under the picker: "Both are kept, with your name and the time." The confirm button is disabled until the rule is met, and the reason is shown in words beside it.
- [x] 1.6 **`Sheet`** (ui-direction §5 C1): a 720 px right overlay on `DrawerFrame`'s motion tokens. Props `items: { id; title }[]`, `index`, `onIndex`, `onClose`, `render(id)`, `primary?` ("Open workspace"). ↑/↓ (and on-screen buttons) move through the list the caller passes (the filtered list), "n of m" in the header, Esc closes, focus is trapped and returns to the row. Full screen below 900 px.
- [x] 1.7 **`CoverageBar`**: `segments: { label; value: MoneyVM | number; tone }[]`, shares in words for screen readers ("Self-performed 49.7%, subcontract 32.3%, …"), labels outside the bar when a segment is under 8%.
- [x] 1.8 **`ThresholdBar`**: `value`, `band?` (±), `threshold`, `max = 100`, `masked?` (renders `Masked` in place of the bar). Text form beside it: "58 ± 8 · threshold 50".
- [x] 1.9 **`EligibilityLine`**: one PQ line from 007a's `EligibilityLine` type: requirement, `SourceChip` for its page, the result as a word (Pass / At risk / Interpretation / Fail / Not stated, via `StatusPill`'s tone vocabulary), evidence chips (credentials), the explanation, and an `actions` slot. Don't compute the state; render 007a's.
- [x] 1.10 **`RequestButton`** (spec §5.5): props `tenderId`, `to: Person`, `topic`, `what`, `section?`, `due` (the caller computes it on the tenant calendar). Click writes `requestWrite(...)` from `domain/gcc/requestKeys.ts` through `mark(key, msg, tone, value)` and `logAudit`. Afterwards it reads `requestFor(...)` and shows "Requested {time} · due {When short}" and is disabled. The toast names the person: "Request sent to Joseph Mathew, due Tue 10 Mar." Under View as it is disabled with "View as is read only."
- [x] 1.11 **`AuditEntry`**: actor name and role, time (`When`), action, `before → after` when given, and a detail line. It's used by the audit tab (Phase 4) and, later, the admin audit log (010).
- [x] 1.12 **`LangBadge`**: `EN` / `AR` / `EN+AR`, with `title` "Document language". **`Tabs`**: an accessible tab list (`role="tablist"`, arrow keys, `aria-selected`, optional badge per tab), if `components/ui` has none. Check first; reuse it if it exists.
- [x] 1.13 `/dev/kit`: one section per component above, on fixture data, light and dark.
...
- [x] 2.1 **`tabs/types.ts`:**
  ...
  ```
- [x] 2.2 **`tabs/index.ts`:** collect `./*.tab.tsx` with `import.meta.glob({ eager: true })`, sorted by `order`; a duplicate `id` throws in dev (the same rule as the other registries). Reserve the ids and orders in a comment, so parallel lanes don't collide:
...
- [x] 2.3 **`Workspace.tsx`** (route `/tenders/:id`):
  - [x] 2.3.1 The row from `dataPort().rows(tenant, { kind: 'all' }, person, 'all', state.done)`, and the tracker from `.tracker(tenant, id, person, state.done)`. **Always pass `state.done`**, and include it in every `useMemo` dependency list, so actions taken in the demo show here (plan 021, running in parallel, applies them behind the port). Any lifecycle query goes through `queriesFor({ tenant, viewer, done })` from `domain/gcc/lifecycle.port.ts`, never unbound. Not found (or restricted and not cleared) → the existing `EmptyState` text "No tender {id} here. It may belong to another company, or not be shared with you."
  - [x] 2.3.2 The active tab from `?tab=`; unknown or hidden → `overview`. Changing tab updates the URL (`replace`), so a reload and Back behave.
  - [x] 2.3.3 Layout (ui-direction §5 C2): the sticky header; the tab list; main content 8/12 and the rail 4/12 at ≥ 1280 px; below 1280 the rail moves under the content.
  - [x] 2.3.4 Mount `SourceHost` once here.
- [x] 2.4 **`App.tsx`:** point `tenders/:id` at `Workspace` (lazy, as today). Delete `TenderSummary.tsx` after moving `factLabel`.

### Phase 3 — Header and rail (view models in `domain/gcc/workspace/`)
- [x] 3.1 **`header.ts` `workspaceHeader(tenant, row, tracker, done)`** returns the header VM (spec §4.1), read-only derivations:
...
- [x] 3.2 **`WorkspaceHeader.tsx`** renders it: sticky under the top bar, one line of identity and one line of track plus due plus owner, fitting 1280 px without wrapping the title into three lines (ellipsis plus a `title` attribute).
- [x] 3.3 **`rail.ts` `workspaceRail(ctx)`**, four blocks, each omitted when empty:
  - [x] 3.3.1 **Recommendation:**
  ...
  - [x] 3.3.2 **Next actions for me:** collect the action sources through the registry, as `buildDashboard` does, run `actionRows(sources, ctx)` with the viewer's `KpiCtx`, and keep the rows whose `tenderId` is this tender. **Don't define action sources here.** Whatever 015 and 013 register shows up automatically. Empty → "Nothing on this tender needs you right now."
  - [x] 3.3.3 **Key dates:** 007a's `keyDatesFor(tenant, id, done)` for register tenders: the next three dates ahead, with `When` and GCC flags; "All key dates" opens the `dates` tab when it exists.
  - [x] 3.3.4 **Blockers:** `tracker.now.blocker`, plus 007a's `blockingOpen(...)` count ("2 fields still being validated by Aisha Al-Qahtani") while in Stage 1, as `route` callouts.
- [x] 3.4 **`Rail.tsx`** renders the blocks; the recommendation card's actions slot is empty in this plan (the gate screens add buttons).

### Phase 4 — The two tabs this plan owns
- [x] 4.1 **`overview.tab.tsx`** (order 10, always shown):
...
- [x] 4.2 **`audit.ts` `auditTimeline(tenant, tenderId, done, audit, viewer)`** and **`audit.tab.tsx`** (order 100, always shown):
...
- [x] 4.3 **Leave room:** a tab owned by a later lane doesn't exist yet, so it isn't shown. No "coming soon" tabs.
...
- [x] 5.2 `Header.tsx`: render it for GCC tenants in place of the hidden `GlobalSearch`. Restricted T-2026-121 is findable only by cleared people (it isn't in the others' rows).

### Phase 6 — Dev check (`55-workspace.tsx`, about ten rows)
- [x] 6.1 Every registered tab has a unique id and an order from the reserved table.
- [x] 6.2 Hero (Najd `HERO_ID`, T-2026-118) header: value, the addendum badge, and a submission countdown in working days are present; the header VM has no `undefined` strings.
- [x] 6.3 Rail on the hero as its Bid Manager: the Stage 1 recommendation with three reasons and at least one page source.
- [x] 6.4 Rail on T-2026-097 (Stage 3) as the Procurement Lead: the serialised rail and header contain no win, margin or price values (compare against the Head of Tendering's).
- [x] 6.5 Audit timeline on a tender past DG1: it contains the DG1 record with who, when and the decision.
- [x] 6.6 Search: `T-2026-121` isn't in the results for Aisha, and is for Faisal.
- [x] 6.7 `requestKeys`: a `requestWrite` then `requestFor` round-trip on an in-memory `done`.
...
- [x] `/dev/checks` in all five GCC tenants: every panel passes, including 55, with no console errors.
- [x] Click-through at 1440 and 1280, light and dark:
...
- [x] Every dashboard's "Open tender" and the tracker's button land on the workspace (check one per dashboard that exists when you finish).
- [x] Reset demo clears a `request:` key written from `/dev/kit`.
- [x] gen-in unchanged (its search, drawer and pages).
- [x] No numbers typed into pages, no role checks outside `access.ts`, no `Date.now`, `Math.random` or bare `new Date()`.

## Execution report
(Filled in by the executor, 2026-09-26.)
- Changed files:
  - **New kit (`components/tender/`):** `Callout.tsx`, `SourceChip.tsx`, `SourceHost.tsx`, `RecommendationCard.tsx`, `ReasonCodePicker.tsx`, `OverrideModal.tsx`, `Sheet.tsx`, `CoverageBar.tsx`, `ThresholdBar.tsx`, `EligibilityLine.tsx`, `RequestButton.tsx`, `AuditEntry.tsx`, `LangBadge.tsx`, `Tabs.tsx`; styles appended to `tender.css` (plus the `.gs-*` search styles).
  - **New view models (`domain/gcc/workspace/`):** `header.ts`, `rail.ts`, `audit.ts`, `index.ts`.
  - **New pages (`pages/gcc/workspace/`):** `Workspace.tsx`, `WorkspaceHeader.tsx`, `Rail.tsx`, `workspace.css`, `tabs/types.ts`, `tabs/index.ts`, `tabs/overview.tab.tsx` (`factLabel` moved here), `tabs/audit.tab.tsx`.
  - **Search:** `components/layout/GccSearch.tsx` (new).
  - **Dev:** `pages/gcc/dev-checks/55-workspace.tsx` (new, 10 rows in Najd, 9 elsewhere); `pages/gcc/dev/KitPreview.tsx` (a `KitPart2` section, inside `SourceHost`) and `pages/gcc/dev/fixtures.ts` (source, override and audit fixtures appended).
  - **Shared, one change each:** `App.tsx` (lazy import plus the `tenders/:id` route); `pages/gcc/screens.ts` (`sub: 'Tender workspace'`); `components/layout/Header.tsx` (import plus `gcc ? <GccSearch /> : <GlobalSearch />`).
  - **Deleted:** `pages/gcc/TenderSummary.tsx` (working tree only; nothing imports it).
- Verification:
  - `npm --prefix app run typecheck` and `npm --prefix app run build` pass (only the existing chunk-size warning).
  - `/dev/checks` in all five GCC tenants: every panel passes, including 55, with 0 console errors.
  - Headless Chromium click-throughs at 1440 and 1280 px, light and dark (16/16 each, twice, 0 console errors):
    - **Head of Tendering, Najd:** ⌘K finds the hero and Enter opens it. The header shows SAR 480.0 M, the working-day countdown and Two-envelope. The p. 39 chip opens the booklet at page 39 with "SAR 1,200,000,000" highlighted; Esc returns focus to the chip. The Decisions & audit tab survives a reload.
    - **Procurement Lead:** T-2026-097 shows "Masked for your role" in every win or price slot, with no "From Stage 3" and no figures. The T-2025-329 blocker reads "Margin below the minimum (figures masked for your role)".
    - **Coordinator:** T-2026-121 isn't found, and its URL shows the not-found state. Faisal finds it.
    - **View as Omar:** the rail's actions are disabled, with "View as is read only."
    - **Qurain:** KWD 39.3 M and AST.
  - "Open tender" lands on the workspace from:
    - the HoT, Bid Manager, CEO, Coordinator and Procurement homes;
    - `/stages/3`;
    - an action row's route.
  - A `request:` key written from `/dev/kit` survives a reload, and Reset demo clears it. gen-in keeps its own search.
  - Kit: the sheet moves focus in, ↑/↓ shows "n of m", Esc returns focus, and the override confirm stays locked until a reason is given.
  - Screens reviewed visually at 1440 light and 1280 dark.
  - A grep of the plan's files finds no `Date.now`, `Math.random`, bare `new Date()` or role checks.
- Deviations from plan:
  - **6.2:** the seed has no addendum on the hero; only ADD-097-2 exists. The check expects no badge on the hero and "Addendum 2 applied" on T-2026-097.
  - **6.3:** the "at least one page source" check runs in Najd only; the other tenants' hero recommendations cite credentials and calcs, not pages.
  - **Rail action sources:**
    - They come from the viewer's own dashboards (home plus the stages they may view), not from every registered source.
    - Every source would have shown 015's `dg2.approve` row, including "n of 5 positions · quorum needs 3", to the Procurement Lead.
    - Rows waiting on someone else sit under a "Waiting on others" sub-heading.
  - **Rail order:** recommendation, then blockers, then next actions, then dates, so a hard block is seen before the actions it stops.
  - **Kit additions:**
    - `WorkspaceCtx` gained `audit` and `hasTab`;
    - `RecommendationCard` gained `reasonsMasked` (a pack the viewer can't open);
    - `SourceChip` also takes `detail` and `terms`.
  - **Overview:**
    - The tracker's own heading is hidden inside the workspace, because the header already says it.
    - Step facts that are null and not masked are left out (a fee once documents are downloaded).
  - **Audit timeline:** in-stage step entries are added, so a tender's history isn't just stage moves.
  - **Highlight terms:** page-chip terms leave out currency codes, which would otherwise light up every "(SAR)" column header.
  - **Search focus:** the search input focuses as it mounts (`autoFocus`), so keys typed straight after ⌘K are never lost.
- Blockers / questions: none open.
  - Earlier in the run, typecheck errors appeared in other plans' files (`metrics/steps.metric.ts`, `dev-checks/45-demo-state.tsx`, `dev-checks/70-stage1.tsx`, `dg1/record.ts`). They cleared on their own; typecheck is clean at hand-off.
- Follow-ups noticed (not done):
  - **015 `dg2.approve` source:** it shows the DG2 positions text to anyone with `tender.view` on any dashboard that lists it; it should mask without `see.positions`.
  - **Flags:** `countryCodeOf` covers GCC countries only, so a place in Jordan or Lebanon gets no flag.
  - **Waiting rows:** a "Waiting on others" row whose route is this same page shows no button (hidden on purpose); 015 or 013 might prefer a different target there.
  - **Not applicable lines:** `EligibilityLine` words 007a's `na` as "Not stated", per the vocabulary, even when the line doesn't apply. 007a could split the two.
