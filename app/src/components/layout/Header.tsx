...
import { ROLES, TENANT, roleOf } from '@/data/roles';
...
function usePageHead() {
  ...
  const heads: Record<string, { title: string; sub: string }> = {
    dashboard: { title: role.view, sub: `${role.title}, ${role.name}` },
    ...
    settings: { title: 'Settings', sub: 'Sources, gates, model routing, appearance and who holds which role.' },
  };
    ...
    <header className="header">
      ...
      <div className="hd-actions">
        ...
        <span className="pop-anchor" ref={profileRef}>
          <button type="button" className={`profile-btn ${profile ? 'open' : ''}`} onClick={() => { setProfile(!profile); setNotif(false); }} aria-haspopup="menu" aria-expanded={profile} aria-label={`Signed in as ${role.name}, ${role.title}. Switch view`}>
            <span className="txt">
              <span className="who">{role.name}</span>
              <span className="what">{role.title}</span>
            </span>
